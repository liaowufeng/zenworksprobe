			...
			<c:otherwise>
				...
				<%--<c:choose>
					...
					<li id="appStop" ${app.available ? '' : 'style="display: none;"'}>
						<c:choose>
							...
							<c:otherwise>
								<a href="<c:url value='/app/stop_summary.htm'><c:param name='webapp' value='${param.webapp}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>">
									<spring:message code="probe.jsp.app.summary.menu.stop"/>
								</a>
							</c:otherwise>
					...
					<li id="appStart" ${app.available ? 'style="display: none;"' : ''}>
						<c:choose>
							...
							<c:otherwise>
									<a href="<c:url value='/app/start_summary.htm'><c:param name='webapp' value='${param.webapp}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>">
										<spring:message code="probe.jsp.app.summary.menu.start"/>
									</a>
							</c:otherwise>
						</c:choose>
					...
					<li id="appReload">
						<c:choose>
							...
							<c:otherwise>
								<a href="<c:url value='/app/reload_summary.htm'><c:param name='webapp' value='${param.webapp}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>">
									<spring:message code="probe.jsp.app.summary.menu.reload"/>
								</a>
							</c:otherwise>
					</c:choose>
					...
					<li id="appUndeploy">
						<c:choose>
							...
							<c:otherwise>
									<a href="<c:url value='/adm/undeploy_summary.htm'><c:param name='webapp' value='${param.webapp}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>" onclick="return confirm('${confirmMessage}')">
									<spring:message code="probe.jsp.app.summary.menu.undeploy"/>
								</a>
							</c:otherwise>
						</c:choose>
				...
				<div class="embeddedBlockContainer">
                    ...
                    <c:otherwise>
						...
						<%-- <spring:message code="probe.jsp.app.summary.servletCount"/>&nbsp;<span class="value"><a href="<c:url value='/servlets.htm'><c:param name='webapp' value='${app.name}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>">${app.servletCount}<span id="servletCount"></span></a></span> --%>
						<spring:message code="probe.jsp.app.summary.servletCount"/>&nbsp;<span class="value"><a href="<c:url value='/servlets.htm'><c:param name='webapp' value='${app.name}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>">${app.servletCount}</a></span>
				...
				<script type="text/javascript">
					...
					function updateAppInfo() {
						...
						'<c:url value="/appruntimeinfo.ajax?${pageContext.request.queryString}"><c:param name='sessionId'  value='${sessionId}'/></c:url>',
						{asynchronous: false});
				...
				<c:if test="${app.available}">
					...
					<script type="text/javascript">
						...
						new Ajax.PeriodicalUpdater('dd-req', '<c:url value="/appreqdetails.ajax"/>?webapp=${app.name}&sessionId=${sessionId}', {frequency: 3});
						new Ajax.PeriodicalUpdater('dd-proc_time', '<c:url value="/appprocdetails.ajax"/>?webapp=${app.name}&sessionId=${sessionId}', {frequency: 3});
					</script>
				</c:if>
