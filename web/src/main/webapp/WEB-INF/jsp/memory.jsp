...
<html>
	...
	<c:choose>
		...
		<c:otherwise>
			...
			<div class="memory">
				<div id="msg" class="helpMessage" style="display: none;">
			...
			<h3><spring:message code="probe.jsp.memory.h3.table"/></h3>
				...
				<%-- <c:choose>
                    ...
                    <c:otherwise>
					<li id="adviseFin">
						<a href="<c:url value='/adm/advisegc.htm?fin=true'><c:param name='sessionId' value='${sessionId}'/></c:url>">
					...
					<li id="dumpHeap">
						<a onclick="return dumpPath('<c:url value="/adm/dumpheap.ajax?sessionId=${sessionId}"/>');" href="#">
							<spring:message code="probe.jsp.memory.dump.heap"/>
						</a>
					</li>
					
					<li id="adviseGC">
					
						<a href="<c:url value='/adm/advisegc.htm'><c:param name='sessionId' value='${sessionId}'/></c:url>">
					...
					<c:forEach items="${pools}" var="pool" varStatus="status">
						...
						<div class="memoryChart" id="${pool.id}" style="${style}">
							<dl>
								<dt>
									${pool.name}
									<c:url var="toggleUrl" value="/remember.ajax?cn=mem_${pool.id}"><c:param name='sessionId' value='${sessionId}'/></c:url>
			...
			<script type="text/javascript">
				...
				function dumpPath(url){
					var help_container = 'msg';
					...
					helpTimerID = setTimeout('Effect.Fade("' + help_container + '")', 55000);
					return false;
				}
