...
<html>
	...
	<body>
				...
				<display:table name="threads" uid="th" class="genericTbl" cellspacing="0" requestURI="" defaultsort="4"
					...
					<display:column class="leftMostIcon" title="&nbsp;">
						...
						<a class="imglink"
								...
								href="<c:url value='/adm/kill.htm'><c:param name='thread' value='${th.name}'/><c:param name='sessionId'  value='${sessionId}'/></c:url>">
					...
					<display:column sortable="true" sortProperty="classLoader" titleKey="probe.jsp.threads.col.classLoader" style="white-space:nowrap;">
						<c:set var="clUrl">
							<c:url value="/cldetails.ajax?thread=${th.name}"><c:param name='sessionId'  value='${sessionId}'/></c:url>
						</c:set>
		...
		<script type="text/javascript">
			setupHelpToggle('<c:url value="/help/threads.ajax"><c:param name='sessionId' value='${sessionId}'/></c:url>');
		</script>
