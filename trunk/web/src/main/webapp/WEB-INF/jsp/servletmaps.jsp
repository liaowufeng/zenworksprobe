		...
		<ul class="options">
			...
			<c:if test="${not empty param.webapp}">
				<li id="viewAllServletMaps">
					<c:url value="/servletmaps.htm" var="allservletmaps" scope="page" ><c:param name='sessionId' value='${sessionId}'/></c:url>
					...
					<display:table name="servletMaps" uid="svlt"
						...
						<c:if test="${empty param.webapp}">
							<display:column sortProperty="applicationName" sortable="true"
								...
								<a href="<c:url value='/appsummary.htm'><c:param name='webapp' value='${svlt.applicationName}'/><c:param name='sessionId' value='${sessionId}'/></c:url>">
